...
/** facebook 确认框插件属性 */
export interface FbCheckboxAttrs
  extends Pick<CheckboxData, 'allowLogin' | 'size' | 'skin' | 'centerAlign'> {
  /** 插件加载网址的基域 */
  origin: string;
  /** Facebook 应用编号 */
  ...
  parse(focus = false) {
    ...
    if (!alreadyRender) {
      window.FB.Event.subscribe('messenger_checkbox', (ev: CheckboxEvent) => {
        if (!ev.ref) {
          warn(
            `Can not found 'ref' attrubite in '${this.name}' Plugin with id ${this.id}`,
            true
          );
          return;
        }
